...
def glazing(message):
    ...
    keyboard2.add(button_glazing_kvartira, button_glazing_dom, button_glazing_office, button_glazing_other,
                  button_glazing_back)
...
def glazing_type(message):
    ...
    button_glazing_back = telebot.types.KeyboardButton(text="🔙 Назад")
    keyboard3.add(button_glazing_kv_window, button_glazing_balcony_block, button_glazing_enter_block,
                  button_glazing_back)
...
def glazing_profile(message):
    ...
    keyboard4.add(button_glazing_rehau, button_glazing_kbe, button_glazing_brusbox, button_glazing_consultation,
                  button_glazing_profile_back)
...
def contacts(message):
    ...
    bot.register_next_step_handler(message, handle_contacts_back)
...
def portfolio(message):
    ...
    bot.register_next_step_handler(message, handle_portfolio_back)


def handle_contacts_back(message):
...
def handle_portfolio_back(message):
    if message.text == "🔙 Главное меню":
        menu(message)
    else:
        portfolio(message)
...
def adress(message):
    if message.text == "🔙 Назад":
        # Go back to the previous step based on context
        # For now, we'll go back to glazing_size or balcony_size
        # You might want to store the user's path to handle this better
        glazing_size(message)
        return
...
def enter_username(message):
    ...
    button_contacts_back = telebot.types.KeyboardButton(text="🔙 Назад")
    button_finish = telebot.types.KeyboardButton(text="✅ Завершить")
    keyboard15.add(button_contacts_back, button_finish)
...
def finish_order(message):
    if message.text == "🔙 Назад":
        enter_username(message)
        return
    elif message.text == "✅ Завершить":
        chat_id = message.chat.id
        keyboard_final = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        button_main_menu = telebot.types.KeyboardButton(text="🔙 Главное меню")
        keyboard_final.add(button_main_menu)
        bot.send_message(chat_id, 'Спасибо! Ваша заявка принята. Мы свяжемся с вами в ближайшее время.',
                         reply_markup=keyboard_final)
        bot.register_next_step_handler(message, handle_final_back)
    else:
        # Store the username and finish
        chat_id = message.chat.id
        keyboard_final = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        button_main_menu = telebot.types.KeyboardButton(text="🔙 Главное меню")
        keyboard_final.add(button_main_menu)
